...
import rospy
import actionlib
from opendr_control.msg import PickAction, PlaceAction, PlaceGoal, PickActionResult
...
class PickAndPlaceClient(object):
    ...
    def give(self, pose_msg):
        try:
            goal = PlaceGoal(pose=pose_msg)
            self.give_client.send_goal(goal,
                                        active_cb=self.place_active,
                                        feedback_cb=self.place_feedback,
                                        done_cb=self.place_done)
            self._loginfo('give has been sent')
        except Exception as e:
            print(e)
            self._loginfo('Error sending give')
    ...
    def pick_and_give(self, pick_goal, place_goal):
        self.pick(pick_goal)
        pick_result = rospy.wait_for_message("/opendr/pick/result",
                                             PickActionResult)
        if pick_result.result.success:
            self.give(place_goal)
            rospy.wait_for_message("/opendr/give/result", PickActionResult)
